...
template<typename T>
T GaussTransformCorr(const vnl_matrix<T>& A,
	const vnl_matrix<T>& B, 
	const vnl_matrix<T>& CorrA, const vnl_matrix<T>& CorrB,
	T scale, T lambda);

template<typename T>
T GaussTransformCorr(const vnl_matrix<T>& A,
	const vnl_matrix<T>& B,
	const vnl_matrix<T>& CorrA,
	const vnl_matrix<T>& CorrB,
	T scale, T lambda,
	vnl_matrix<T>& gradient);
...
template<typename T>
T GaussTransformCorr(const T* A, const T* B, const T* CorrA, const T* CorrB,
	int m, int n, int numCorr, int dim, T scale, T lambda) {
	T cross_term = 0;
	scale = SQR(scale);
	for (int i = 0; i < m; ++i) {
		for (int j = 0; j < n; ++j) {
...
template<typename T>
T GaussTransformCorr(const T* A, const T* B, const T* CorrA, const T* CorrB,
	int m, int n, int numCorr, int dim, T scale, T lambda, T* grad) {
	int indi, indj;
	T cross_term = 0;
	for (int i = 0; i < m * dim; ++i) {
		grad[i] = 0;
	...
	for (int i = 0; i < numCorr; ++i) {
			...
			for (int d = 0; d < dim; ++d) 
			{
				dist_ij += SQR(A[indi*dim + d] - B[indj*dim + d]);
			}
			T cost_ij = exp(-1.0 * dist_ij / scale);
			for (int d = 0; d < dim; ++d) 
			{
				grad[indi*dim + d] -= cost_ij * 2.0 * (A[indi*dim + d] - B[indj*dim + d]);
			}
			cross_term += cost_ij;
		}
		/* printf("cross_term = %.3f\n", cross_term);  */
...
template<typename T>
T GaussTransformCorr(const vnl_matrix<T>& A,
	const vnl_matrix<T>& B, const vnl_matrix<T>& CorrA,
	const vnl_matrix<T>& CorrB, T scale, T lambda) {
	// assert A.cols() == B.cols()
	return GaussTransformCorr(A.data_block(), B.data_block(), CorrA.data_block(), CorrB.data_block(),
		A.rows(), B.rows(), A.cols(), scale, lambda);
}
